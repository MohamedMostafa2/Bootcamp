/*H**********************************************************************
* FILENAME :DIO.c
*
* DESCRIPTION :
*       Digital Input Output Driver 
*
* PUBLIC FUNCTIONS :
*       void DIO_WritePin(uint8 PinNum,uint8 PinValue)
*       uint8 DIO_ReadPin(uint8 PinNum)
*       DIO_SetPinDirection(uint8 PinNum,uint8 PinDirection)
*
* NOTES :
*       These Driver For MicroController DIO
*
*
* AUTHOR :    Mohamed AbdelKarim        START DATE :    19 Feb 19
*
* CHANGES : None
*
*
*H*/

#include "../Includes/Types.h"
#include "../Includes/DIO/BitwiseOperations.h"
#include "../Includes/DIO/DIO.h"


/************************************************************************
* Function Name : DIO_WritePin
*
* Parameter
*			I/P : PinNum , PinValue
*			O/P : None
*			I/O : None
*
* Return : None
*
* Description : Set (High) or Clear (Low) to Specific Pin             
*
************************************************************************/
void DIO_WritePin(uint8 PinNum,uint8 PinValue){
	
	/* Check If  Pin0 <= PinNum <= Pin7  */
	if( (PinNum >= Pin0) && (PinNum <= Pin7) ){
		if(PinValue == LOW){ /* Check If Pin Value = LOW */
			Clear_Bit(PORTA,PinNum); /* Clear Bit (PinNum) on PortA Register  */
				                     /* Ex. PinNum = 5 ,Then Clear Bit 5 on PortB */
		}
		else{ /* Default State is Set Bit*/
			Set_Bit(PORTA,PinNum); /* Set Bit (PinNum) on PortA Register  */
			                       /* Ex. PinNum = 3 ,Then Clear Bit 3 on PortB */
		}
	}
	/* Check If  Pin8 <= PinNum <= Pin15  */
	else if( (PinNum >= Pin8) && (PinNum <= Pin15) ){
		if(PinValue == LOW){ /* Check If PinValue = LOW */
			Clear_Bit(PORTB,PinNum-Pin8); /* Clear Bit (PinNum - 8) on PortB Register  */
										  /* Ex. PinNum = 9 ,Then Clear Bit 1 on PortB */
		}
		else{ /* Default State is Set Bit*/
			Set_Bit(PORTB,PinNum-Pin8); /* Set Bit (PinNum - 8) on PortB Register  */
		}                               /* Ex. PinNum = 10 ,Then Set Bit 2 on PortB */
	}
	/* Check If  Pin16 <= PinNum <= Pin23  */
	else if( (PinNum >= Pin16) && (PinNum <= Pin23) ){
		if(PinValue == LOW){ /* Check If PinValue = LOW */
			Clear_Bit(PORTC,PinNum-Pin16); /* Clear Bit (PinNum - 16) on PortC Register  */
		}                                  /* Ex. PinNum = 20 ,Then Clear Bit 4 on PortC */
		else{ /* Default State is Set Bit*/
			Set_Bit(PORTC,PinNum-Pin16);   /* Set Bit (PinNum - 16) on PortC Register  */
		}                                  /* Ex. PinNum = 21 ,Then Set Bit 5 on PortB */
	}
	/* Check If  Pin24 <= PinNum <= Pin31  */
	else if( (PinNum >= Pin24) && (PinNum <= Pin31) ){
		if(PinValue == LOW){ /* Check If PinValue = LOW */
			Clear_Bit(PORTD,PinNum-Pin24); /* Clear Bit (PinNum - 23) on PortD Register  */
		}                                  /* Ex. PinNum = 25 ,Then Clear Bit 2 on PortC */
		else{ /* Default State is Set Bit*/
			Set_Bit(PORTD,PinNum-Pin24);  /* Set Bit (PinNum - 23) on PortD Register  */
		}                                 /* Ex. PinNum = 23 ,Then Set Bit 0 on PortB */
	}
	else{
		
	}
	
}


/************************************************************************
* Function Name : DIO_ReadPin
*
* Parameter
*			I/P : PinNum
*			O/P : None
*			I/O : None
*
* Return : Value of PinNum
*
* Description : Get Value of Specific Pin (High/Low)
*
************************************************************************/
uint8 DIO_ReadPin(uint8 PinNum){
	
	if( (PinNum >= Pin0) && (PinNum <= Pin7) ){ /* Check If  Pin0 <= PinNum <= Pin7  */
		return Get_Bit(PINA,PinNum);  /* Return The Value of Pin (PinNum) From PinA Register*/
		                              /* Ex. PinNum = 2 ,Then Get The Value of Pin 2 From PinA Register */
	}
	else if( (PinNum >= Pin8) && (PinNum <= Pin15) ){ /* Check If  Pin8 <= PinNum <= Pin15  */
		return Get_Bit(PINB,PinNum-Pin8); /* Return The Value of Pin (PinNum - 8) From PinB Register*/
		                                  /* Ex. PinNum = 9 ,Then Get The Value of Pin 1 From PinB Register */
	}
	else if( (PinNum >= Pin16) && (PinNum <= Pin23) ){ /* Check If  Pin16 <= PinNum <= Pin23  */
		return Get_Bit(PINC,PinNum-Pin16); /* Return The Value of Pin (PinNum - 16) From PinC Register*/
		                                   /* Ex. PinNum = 18 ,Then Get The Value of Pin 2 From PinC Register */
	}
	else if( (PinNum >= Pin24) && (PinNum <= Pin31) ){ /* Check If  Pin24 <= PinNum <= Pin31  */
		return Get_Bit(PIND,PinNum-Pin24); /* Return The Value of Pin (PinNum - 23) From PinD Register*/
		                                   /* Ex. PinNum = 26 ,Then Get The Value of Pin 3 From PinD Register */
	}
	else{
		/* Do Nothing */
	}
	
	return LOW; /* Default State if PinNum is Out of Range */
}

/************************************************************************
* Function Name : DIO_SetPinDirection
*
* Parameter
*			I/P : PinNum , PinDirection
*			O/P : None
*			I/O : None
*
* Return : None
*
* Description : Set Direction To Specific Pin (Input / Output)
*
************************************************************************/
void DIO_SetPinDirection(uint8 PinNum,uint8 PinDirection){
	
	if( (PinNum >= Pin0) && (PinNum <= Pin7) ){ /* Check If  Pin0 <= PinNum <= Pin7  */
		if(PinDirection == INPUT){ /* Check If Pin Direction = Input */
			Clear_Bit(DDRA,PinNum); /* Clear Bit (PinNum) on DDRA Register  */
		}                           /* Ex. PinNum = 2 ,Then Clear Bit 2 on DDRA Register */
		else{ /* Default State is Set Bit Direction to Output */
			Set_Bit(DDRA,PinNum);  /* Set Bit (PinNum) on DDRA Register  */
		}                          /* Ex. PinNum = 5 ,Then Set Bit 5 on DDRA Register */
	}
	else if( (PinNum >= Pin8) && (PinNum <= Pin15) ){ /* Check If  Pin8 <= PinNum <= Pin15  */
		if(PinDirection == INPUT){  /* Check If Pin Direction = Input */
			Clear_Bit(DDRB,PinNum-Pin8); /* Clear Bit (PinNum - 8) on DDRB Register  */
		}                                /* Ex. PinNum = 9 ,Then Clear Bit 1 on DDRB Register */                       
		else{ /* Default State is Set Bit Direction to Output */
			Set_Bit(DDRB,PinNum-Pin8);  /* Set Bit (PinNum - 8) on DDRA Register  */
		}                               /* Ex. PinNum = 10 ,Then Set Bit 2 on DDRA Register */
	}
	else if( (PinNum >= Pin16) && (PinNum <= Pin23) ){ /* Check If  Pin16 <= PinNum <= Pin23  */
		if(PinDirection == INPUT){  /* Check If Pin Direction = Input */
			Clear_Bit(DDRC,PinNum-Pin16);/* Clear Bit (PinNum - 16) on DDRC Register  */
		}                                /* Ex. PinNum = 17 ,Then Clear Bit 1 on DDRC Register */
		else{ /* Default State is Set Bit Direction to Output */
			Set_Bit(DDRC,PinNum-Pin16);  /* Set Bit (PinNum - 16) on DDRA Register  */
		}                                /* Ex. PinNum = 18 ,Then Set Bit 2 on DDRA Register */
	}
	else if( (PinNum >= Pin24) && (PinNum <= Pin31) ){ /* Check If  Pin24 <= PinNum <= Pin31  */
		if(PinDirection == INPUT){  /* Check If Pin Direction = Input */
			Clear_Bit(DDRD,PinNum-Pin24);/* Clear Bit (PinNum - 24) on DDRD Register  */
		}                                 /* Ex. PinNum = 25 ,Then Clear Bit 1 on DDRD Register */
		else{ /* Default State is Set Bit Direction to Output */
			Set_Bit(DDRD,PinNum-Pin24);  /* Set Bit (PinNum - 24) on DDRA Register  */
		}                                /* Ex. PinNum = 26 ,Then Set Bit 2 on DDRA Register */
	}
	else{
		/* Do Nothing */
	}
	
	
}
